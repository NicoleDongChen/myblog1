...
//---------点赞------------
router.get('/api/addcount/:id',function(req,res){
    db.Article.update({_id :req.params.id},function(err,docs) {
           db.Article.find({_id: req.params.id}, function (err, docs) {
               if (err) {
                   return
               }
               console.log(docs);
               // docs[0].count = req.body.obj.count + 1;
               //db.Article(docs[0]).save(function (err) {
               //    if (err) {
               //        res.status(500).send();
               //        return
